...
**Usage example**:
Below is an image of the log-likelihood terms training an importance weighted autoencoder on MNIST using binomial sampling before each epoch. The training is done using one Monte-carlo sample to approximate the expectations over q(z|x) and one importance weighted sample.
