...
class NormalizingPlanarFlowLayer(lasagne.layers.Layer):
    """
        Normalizing Planer Flow Layer as described in Rezende et. al. [REZENDE] (
        Equation numbers and appendixes refers to this paper)
        Eq. (8) is used for calculating the forward transformation f(z). The last
        term of Eq (13) is also calculated within this layer and returned as an
        output for computational reasons. Further the transformaton is ensured to be
        invertible using the constrains described in appendix A.1
    ...
    def __init__(self, incoming, u=lasagne.init.Normal(),
        ...
        self.b = self.add_param(b, tuple(), name="b")
    ...
    def get_output_for(self, input, **kwargs):
        ...
        psi = T.dot( (1-lasagne.nonlinearities.tanh(zwb)**2).dimshuffle(0,'x'),  self.w.dimshuffle('x',0))
